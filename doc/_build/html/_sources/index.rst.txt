...
The coin example :cite:`VenVer04ICLP04IC` is represented as (file `coin.pl <http://cplint.eu/e/coin.pl>`__) ::
...
where the :math:`p_i` are numeric expressions.
...
:code:`mcintyre` has the parameters <TODO - translate in list> ::
...
Decision Theory
---------------
cplint allows also to represent decision theory problem following the syntax of Dtproblog [ADD REFERENCE].

Decision facts can be expressed with the operator :code:`?::` (see [TODO ADD LINK EXAMPLE UMBRELLA]) ::

	? :: umbrella.

Utility facts can be expressed with :code:`utility/2` where the first parameter is the fact and the second its utility (see [TODO ADD LINK EXAMPLE UMBRELLA]) ::
...
Then, you have to specify the language bias by means of mode declarations in the style of `Progol <http://www.doc.ic.ac.uk/\string ~shm/progol.html>`_. ::
