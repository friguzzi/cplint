You can ask the probability of an atom using \verb|pita| using the predicate using \verb| prob(:Query:atom,-Probability:float)|
...
\verb|L| is the list of values of \verb|Arg| obtained with
\verb|findall(Arg,Query,L)| and \verb|N|
is the number of samples
returning that list of values.
If \verb|L| is the empty list, it means that for that
sample the query failed. If, in all couples \verb|L-N|, \verb|L| 
is a list with a 
