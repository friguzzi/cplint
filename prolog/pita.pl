...
:- module(pita,[s/2, prob/2, set_pita/2,setting_pita/2,
   one/2,zero/2,and/4,or/4,bdd_not/3,init/3,init_bdd/2,init_test/1,
...
get_node(Goal,Env,B):-
  setting_pita(depth_bound,true),!,
  setting_pita(depth,DB),
  retractall(v(_,_,_)),
...
process_head_ground([Head:ProbHead], Prob, [Head:ProbHead1|Null]) :-!,
  ...
  setting_pita(epsilon_parsing, Eps), 
  EpsNeg is - Eps, 
  ProbLast > EpsNeg, 
  (ProbLast > Eps ->
    Null = ['':ProbLast]
...
set_pita(Parameter,Value):-
  retract(setting_pita(Parameter,_)),
  assert(setting_pita(Parameter,Value)).
