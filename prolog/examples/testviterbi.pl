...
*/
test((viterbi(win,P,Exp),close_to(P,0.36),
Exp=[
  rule(0, red, [red:0.4, '':0.6], []),
  rule(1, green, [green:0.9, '':0.09999999999999998], [])]),vit_win).

test((viterbi(hmm1(S,[a,g,g]),P,_Exp),close_to(P,0.000405),
  S = [q2, q2, q1]
  ),vit_hmm).

test((viterbi(hmm1(S,[a,a,a]),P,_Exp),close_to(P,0.0008000000000000003),
  S = [q1, q1, q1]
  ),vit_hmm).

test((viterbi(heads(coin),Prob,Exp),close_to(Prob,0.45),
Exp = [rule(0, heads(coin), [heads(coin):0.5, tails(coin):0.5], [toss(coin), \+biased(coin)]),
	rule(2, fair(coin), [fair(coin):0.9, biased(coin):0.1], [])]),
	vit_coin).

test((viterbi(color(s,purple),Prob,Exp),close_to(Prob,0.5),
	Exp = [rule(0, cg(s, 1, p), [cg(s, 1, p):0.5, cg(s, 1, w):0.5],
	 [mother(m, s), cg(m, 1, p), cg(m, 2, w)])]),
 	vit_mendel).


test((viterbi(color(s,white),Prob,Exp),close_to(Prob,0.25),
